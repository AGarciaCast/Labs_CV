...
def Lv(inpic, shape = 'same', sigma=None, sobel=False, reduced=True):
    
    if sigma is not None:
        inpic = gaussfft(inpic, sigma)
    
    Lx = convolve2d(inpic, deltax(sobel, reduced), shape)
    Ly = convolve2d(inpic, deltay(sobel, reduced), shape)
...
def ex4():
    ...
    imgs = [np.load("Images-npy/few256.npy"), np.load("Images-npy/godthem256.npy")]
    ...
    for img in imgs:
        ...
        for j, threshold in enumerate(thresholds):
            for i, scale in enumerate(scales):
                ax = fig.add_subplot(len(thresholds), len(scales), j*len(scales) +i +1)
